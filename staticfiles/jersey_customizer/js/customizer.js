...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try { renderer.toneMappingExposure = 1.0 } catch (e) {}
      ...
      if (config.logo && bodyMeshes.length > 0) {
        ...
        textureLoader.load(config.logo, (texture) => {
          ...
          // helper: create decal on mesh using a raycast to get the surface normal
            function createDecalOnMesh(mesh, texture, preservedQuat, forceCenter = false) {
              try {
                ...
                // If caller requested force-center placement, skip raycasts and place at bbox center toward camera
                if (forceCenter) {
                  console.log('forceCenter placement requested — placing at bbox center toward camera')
                  ...
                  const pointFC = centerWorld.clone().add(frontNormal.clone().multiplyScalar(0.02))
                  const quatFC = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0,0,1), frontNormal)
                  const eulerFC = new THREE.Euler().setFromQuaternion(quatFC)
                  const decalSizeFC = new THREE.Vector3(0.4 * config.logoSize, 0.4 * config.logoSize, 0.4)
                  const decalGeometryFC = new THREE.DecalGeometry(mesh, pointFC, eulerFC, decalSizeFC)
                  const decalMaterialFC = new THREE.MeshBasicMaterial({ map: texture, transparent: true, depthTest: true, depthWrite: false, polygonOffset: true, polygonOffsetFactor: -4, side: THREE.FrontSide })
                  decalMesh = new THREE.Mesh(decalGeometryFC, decalMaterialFC)
                  if (decalGeometryFC && decalGeometryFC.attributes && decalGeometryFC.attributes.position && decalGeometryFC.attributes.position.count > 0) {
                    if (jersey) jersey.add(decalMesh)
                    try { logoMesh = decalMesh; logoMesh.name = 'logoMesh' } catch(e){}
                    console.log('forceCenter decal added at', pointFC.toArray())
                    return
                  } else {
                    // fallback to plane at center
                    try {
                      const w = 0.5 * config.logoSize
                      const h = w
                      const planeGeo = new THREE.PlaneGeometry(w, h)
                      const planeMat = new THREE.MeshBasicMaterial({ map: texture, transparent: true, side: THREE.FrontSide })
                      const plane = new THREE.Mesh(planeGeo, planeMat)
                      plane.position.copy(pointFC)
                      plane.quaternion.copy(quatFC)
                      scene.add(plane)
                      if (jersey) jersey.attach(plane)
                      decalMesh = plane
                      logoMesh = decalMesh
                      console.log('forceCenter plane fallback added at', pointFC.toArray())
                      return
                    } catch (e) { console.warn('forceCenter plane fallback failed', e) }
                  }
                }
                    ...
                    if (uv) {
                      ...
                      if (img) {
                        ...
                        if (painted) {
                          ...
                          try {
                            // create an immediate, visible overlay (small plane) at the hit point so user sees feedback
                            ...
                            // position at actual intersection point and offset slightly along normal to avoid z-fighting
                            if (typeof point !== 'undefined' && point) {
                              const overlayPos = point.clone()
                              try { overlayPos.add((normalWorld || new THREE.Vector3(0,1,0)).clone().multiplyScalar(0.02)) } catch (e) {}
                              overlay.position.copy(overlayPos)
                            } else {
                              overlay.position.copy(worldPos)
                            }
                            // orient overlay to surface normal
                            ...
                            // add to scene (leave in world-space so it precisely matches the hit)
                            scene.add(overlay)
                            // place a small debug sphere at the computed hit point to diagnose placement
                            try { const s = new THREE.Mesh(new THREE.SphereGeometry(0.02, 8, 8), new THREE.MeshBasicMaterial({ color: 0xff00ff })); if (typeof point !== 'undefined' && point) { s.position.copy(point); scene.add(s); setTimeout(()=>{ try{ if (s.parent) s.parent.remove(s); s.geometry.dispose(); s.material.dispose(); }catch(e){} },5000) } } catch(e){}
                            console.log('overlay placed at', (point||worldPos).toArray(), 'localPos', localPos ? localPos.toArray() : null)
                            // remove overlay after a short time so it doesn't persist forever
                            setTimeout(() => { try { if (overlay.parent) overlay.parent.remove(overlay); overlay.geometry.dispose(); overlay.material.dispose() } catch(e){} }, 5000)
          ...
          if (!targetMesh) {
            ...
            createDecalOnMesh(targetMesh, texture, preservedQuat, true)
            if (decalMesh) {
    ...
    function updateElementsVisibility() {
      ...
      if (logoMesh) {
        logoMesh.visible = config.currentView === config.logoPlacement
      }
      ...
      sleeveMeshes.forEach(mesh => {
        try {
          ...
          if (config.jerseyType !== "sleeveless") {
            if (mesh.material && mesh.material.isMeshStandardMaterial) {
              mesh.material.color.set(config.secondaryColor)
              mesh.material.needsUpdate = true
            } else {
              const existingMap = (mesh.material && mesh.material.map) ? mesh.material.map : null
              const mat = new THREE.MeshStandardMaterial({ color: config.secondaryColor })
              if (existingMap) mat.map = existingMap
              mesh.material = mat
            }
          }
